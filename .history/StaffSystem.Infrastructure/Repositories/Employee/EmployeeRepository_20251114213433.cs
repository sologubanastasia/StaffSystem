...
public class EmployeeRepository : IEmployeeRepository
{
    ...
    public IEnumerable<Employee> GetAll(string? department, string? position, string? name)
    {
        ...
        var sql = @"
            ...
            WHERE 
                (@department IS NULL OR d.""DepartmentName"" ILIKE '%' || CAST(@department AS TEXT) || '%')  -- ✅ Виправлення 1
                AND (@position IS NULL OR p.""PositionName"" ILIKE '%' || CAST(@position AS TEXT) || '%')    -- ✅ Виправлення 2
                AND (@name IS NULL OR (e.""LastName"" || ' ' || e.""FirstName"" || ' ' || COALESCE(e.""MiddleName"", '')) ILIKE '%' || CAST(@name AS TEXT) || '%') -- ✅ Виправлення 3
            ORDER BY e.""LastName"", e.""FirstName"";";

        using var command = new NpgsqlCommand(sql, connection);

        // Npgsql автоматично обирає тип, але для ILIKE з NULL ми допомагаємо йому, передаючи object?
