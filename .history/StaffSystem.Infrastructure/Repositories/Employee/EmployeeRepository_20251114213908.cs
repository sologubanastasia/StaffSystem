...
public IEnumerable<Employee> GetAll(string? department, string? position, string? name)
{
    ...
    var sql = @"
        ...
        WHERE 
            (@department IS NULL OR d.""DepartmentName"" ILIKE '%' || COALESCE(@department, '') || '%')
            AND (@position IS NULL OR p.""PositionName"" ILIKE '%' || COALESCE(@position, '') || '%')
            AND (@name IS NULL OR (e.""LastName"" || ' ' || e.""FirstName"" || ' ' || COALESCE(e.""MiddleName"", '')) ILIKE '%' || COALESCE(@name, '') || '%')
        ORDER BY e.""LastName"", e.""FirstName"";";

    using var command = new NpgsqlCommand(sql, connection);

    // 💡 КЛЮЧОВЕ ВИПРАВЛЕННЯ: Використовуємо Add() замість AddWithValue() та явно вказуємо NpgsqlDbType.Text
    command.Parameters.Add(new NpgsqlParameter("@department", NpgsqlTypes.NpgsqlDbType.Text) 
    { 
        Value = (object?)department ?? DBNull.Value 
    });
    command.Parameters.Add(new NpgsqlParameter("@position", NpgsqlTypes.NpgsqlDbType.Text) 
    { 
        Value = (object?)position ?? DBNull.Value 
    });
...
// ... (решта коду репозиторію)
    // Методи GetById та Update залишаються без змін, оскільки в них немає конкатенації з параметрами-рядками.
