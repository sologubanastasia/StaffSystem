...
public class EmployeeRepository : IEmployeeRepository
{
    ...
    public IEnumerable<Employee> GetAll(string? department, string? position, string? name)
    {
        ...
        var sql = @"
            SELECT e.*, d.""DepartmentName"", p.""PositionName"", c.""Name"" AS ""CompanyName""
            FROM ""Employee"" e
            ...
            WHERE (@department IS NULL OR d.""DepartmentName"" ILIKE '%' || @department || '%')
              ...
              AND (@name IS NULL OR (e.""LastName"" || ' ' || e.""FirstName"") ILIKE '%' || @name || '%')
    ...
    public void Update(Employee employee)
    {
        ...
        var sql = @"
            UPDATE ""Employee""
            SET ""FirstName""=@fn, ""LastName""=@ln, ""MiddleName""=@mn, ""Address""=@addr, ""Phone""=@phone, ""BirthDate""=@bd, ""HireDate""=@hd, ""Salary""=@sal,
                ""DepartmentId""=@did, ""PositionId""=@pid, ""CompanyId""=@cid
            WHERE ""EmployeeId""=@id";
        ...
        command.Parameters.AddWithValue("@mn", employee.MiddleName);
